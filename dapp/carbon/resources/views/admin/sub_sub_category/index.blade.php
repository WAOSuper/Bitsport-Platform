...
<div class="page-content-wrapper">
    ...
    <div class="page-content" style="min-height: 1489px;">
        ...
        <ul class="page-breadcrumb breadcrumb">
            ...
            <span class="set-right"><a href="{{route('admin-subsubcategory.create')}}" class="btn btn-primary ">Add Sub Sub Category</a></span>
        </ul>
        <!-- END PAGE BREADCRUMB -->
        <div class="row">
                ...
                <div class="portlet light bordered">
                    ...
                    <div class="portlet-body">
                        ...
                        <table class="table table-striped table-bordered table-hover" id="sample_1">
                            ...
                            <tbody><?php $i = 1; ?>
                          @foreach($subsubcategory as $scat)
                            <tr>
                                <td>{{$i++}}</td>
                                 <td>{{@$scat->subsubcat->name}}</td>
                                 <td>{{@$scat->name}}</td>
                                 <td>{{ @$scat->slug }}</td>
                                   @if(@$scat->status == 1)
                                ...
                                @endif
                                  <td width="250px">
                                        ...
                                        <a href="{{ route('admin-subsubcategory.edit',$scat->id) }}" class="btn btn-xs blue" ><i class="fa fa-pencil"></i></a>

                                        @if($scat->status == 0)
                                        <a href="{{ route('admin-subsubcategory.deactivestatus',$scat->id) }}" class="btn btn-danger" >Deactivate</a>
                                        @endif
                                        @if($scat->status == 1)
                                        <a href="{{ route('admin-subsubcategory.activestatus',$scat->id) }}" class="btn btn-success" >Active</a>
                                        @endif

                                 <!--       {{ Form::open(['method' => 'DELETE', 'class'=>'set-right' ,'route' => ['admin-subsubcategory.destroy',$scat->id]]) }}
