...
<div class="page-content-wrapper">
                    ...
                    <div class="portlet-body table-responsive">
                        ...
                        <table class="table table-striped table-bordered table-hover" id="sample_1">
                            ...
                            <tbody>
                                ...
                                <td><a href="{{url('/')}}/deposit/BTP" class="btn btn-primary ">Deposit</a>
                                ...
                                <td><label><b>Withdraw BTP to  </b></label>
                                    ...
                                    <a href="#" class="btn btn-warning" data-toggle="modal" data-target="#ToConvertpaypal">PayPal</a>
                                </td>    
                                @else    
                                <td><label><b>Send BTP Balance to -</b></label>
                                    ...
                                    <button class="btn btn-warning" disabled="">PayPal</button>
                                @endif    
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td><img src="{{url('/')}}/assets/images/coins/paypal.png" width="40px" alt="paypal"></td>
                                <td>PayPal</td>
                                <td>{{number_format(Sentinel::getUser()->total_paypal_bal,2)}} USD</td>
                                <td><a href="{{url('/')}}/deposit/paypal" class="btn btn-primary ">Deposit</a>
                                    <a class="btn btn-danger" @if($withdrawal->withdrawal == 1) href="{{ url('withdraw/paypal') }}@endif " @if($withdrawal->withdrawal == 0) disabled="" @endif  >Withdraw</button></a>
                                </td>
                                @if(Sentinel::getUser()->total_paypal_bal > 0)
                                <td><a href="{{url('/')}}/btp-convert/paypal" class="btn btn-success" onclick="return confirm('Are you sure You want to Convert PayPal Balance to BTP Balance ?')">Swap All To BTP</a>
                                    <a href="#" class="btn btn-primary" data-toggle="modal" data-target="#Convertpaypal{{$user->id}}">Custom Amount Swap</a>
                                </td>
                                @else
                                    <td><span class="text-danger"> Insufficient PayPal Balance</span></td>
                                @endif
                            </tr>
                        ...
                        <div class="modal fade" id="Convertpaypal{{$user->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-body"><h3>Convert PayPal Balance to BTP Balcance?</h3>
                                        {{Form::open([ 'id'=>'formsubmitpaypal'.$user->id, 'url' => url("addbal/$user->id"), 'method' => 'POST'])}}
                                        <input type="hidden" name="coin_name" value="paypal">
                                            <tr id="paypalTR" class="ccConvert">  
                                                <td><label>Enter PayPal Balance</label><span class="DashtoBtp text-danger"></span>    
                                                    <input type="number" step="0.000000000000001" min="0" name="bal" class="form-control text" id="paypalbtp" oninput="btppaypalConverter(this.value)" onchange="btppaypalConverter(this.value)" autocomplete="off" class="btpText" placeholder="Enter PayPal Amount">
                                                </td>
                                                <td><label>BTP Balance</label>
                                                  <input type="number" class="form-control text" placeholder="BTP Amount" name="BTP" id="btppaypal" value=""class="ccText" oninput="paypalConverter(this.value)" onchange="paypalConverter(this.value)" step="0.00000001" min="0">
                                                </td>
                                            </tr>
                                        {{ Form::close() }}
                                        <div class="modal-footer">
                                            <button form="formsubmitpaypal{{$user->id}}" id="paypaltoBtp" type="submit" class="btn btn-success" >Submit</button>
                                            <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="modal fade" id="ToConvertpaypal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-body"><h3>Convert BTP Balance to PayPal Balance?</h3>
                                        {{Form::open([ 'id'=>'formpaypal'.$user->id, 'url' => url("baladd/$user->id"), 'method' => 'POST'])}}
                                        <input type="hidden" name="coin_name" value="paypal">
                                            <tr id="paypalTR" class="ccConvert">  
                                                <td><label>Enter BTP Balance</label> <span class="Btptopaypal text-danger"></span>
                                                  <input type="number" step="0.01" min="0" class="form-control text" autocomplete="off" placeholder="Enter BTP Amount" name="BTP" value="" oninput="paypal(this.value)" onchange="paypal(this.value)" class="ccText" id="paypal1">
                                                </td>
                                                <td><label>PayPal Balance</label>
                                                    <input type="number" name="add_bal" autocomplete="off" class="form-control text" id="paypalbtpto" class="btpText" placeholder="PayPal Amount" oninput="paypal2(this.value)" onchange="paypal2(this.value)" step="0.01" min="0">
                                                </td>
                                            </tr>
                                        {{ Form::close() }}
                                    <div class="modal-footer">
                                        <button form="formpaypal{{$user->id}}" id="Btptopaypal" type="submit" class="btn btn-success">Submit</button>
                                        <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
                                    </div>
                                    </div>
                                </div>
    ...
    <script>
        function btppaypalConverter(valNum){
            var bal = parseFloat("{{Sentinel::getUser()->total_paypal_bal}}", 10);
            var value = parseFloat(valNum,10);
            if(bal > value){
                var dd = valNum*{{$setting->paypal_price}};
                var data = dd/{{$setting->rate}};
                $('#btppaypal').val(data.toFixed(8));
                $('#paypaltoBtp').attr("disabled", false);
                $('.paypaltoBtp').html('');
            }else{
                $('.paypaltoBtp').html(' Insufficient balance');  
                $('#paypaltoBtp').attr("disabled", true);
            }
        }
    </script>
    ...
    <script>
        function paypalConverter(valNum){
              var data = valNum*{{$setting->rate}}/{{$setting->paypal_price}};
              $('#paypalbtp').val(data.toFixed(8));
    ...
    <script>
        function paypal(valNum){
            var bal = parseFloat("{{Sentinel::getUser()->mbtc}}", 10);
            var value = parseFloat(valNum,10);
            if(bal > value){
                var data = valNum*{{$setting->rate}}/{{$setting->paypal_price}};
                $('#paypalbtpto').val(data.toFixed(8));
                $('#Btptopaypal').attr("disabled", false);
                $('.Btptopaypal').html('');
            }else{
                $('.Btptopaypal').html(' Insufficient balance');  
                $('#Btptopaypal').attr("disabled", true);
            }
        }
    </script>
    <!---   start--->
    ...
    <script>
        function paypal2(valNum){
            var data = valNum*{{$setting->paypal_price}}/{{$setting->rate}};      
            $('#paypal1').val(data.toFixed(8));

            var bal = parseFloat("{{Sentinel::getUser()->mbtc}}", 10);
            var value = parseFloat(data,10);
            if(bal > value){
                $('#Btptopayapl').attr("disabled", false);
                $('.Btptopaypal').html('');
            }else{
                $('.Btptopaypal').html(' Insufficient balance');  
                $('#Btptopaypal').attr("disabled", true);
