            ...
            @if(session('error'))<div class="alert alert-danger">{{ session('error') }}</div><br>@endif
              <div class="tabs main-tab">
                <ul class="nav custom-tab nav-tabs">
                  ...
                  <li><a data-toggle="tab" href="#stats">Sponsor (DeFi)</a></li>
                </ul>
                      ...
                      @endphp
                        <div class="col-md-6 participants">
                          <div class="participants-inner">
                            ...
                            <div class="participant-info"><a href="{{ URL('public_profile').'/'.$participant->user->id }}">{{($participant->user->username != "") ? $participant->user->username : $participant->user->first_name.' '.$participant->user->last_name}}</a></div>
                          </div>
                        </div>
                      @php
                        endforeach;
                                 ...
                                 foreach($participants as $participant):
                                  ...
                                  <tr class="bg-navy-400">
                                    ...
                                    <td>
                                      ...
                                      <span><a href="<?php echo URL('public_profile').'/'.$participant->user->id;?>">{{($participant->user->username != "") ? $participant->user->username : $participant->user->first_name.' '.$participant->user->last_name}}</a></span>
                      ...
                      <div class="empty-message">
						...
						<table style="width:100%" class="table table-bordered mt-10">
							<thead>
							  <tr>
								<th>Learn More</th>
								<th><a href="https://docs.bitsport.gg">docs.bitsport.gg</a></th>
							  </tr>
								</thead>
								<tbody>
								<?php echo $tournament->stats ?>
                <div class="btn-card">
          <a id="sponsor_tournament" class="btn-join" href="<?php if(!empty($sponsers)){ echo $sponsers[0]->link;}?>">Sponsor (Join PreSale)</a>  
                </div>
							 </tbody>					
						 </table>
					 </div>
              ...
              @if($tournament->status == 1 && $tournament->player_limit > $participant_count)
                 <div class="gpg-container">
                    ...
                    <div class="gpg-user">
                       ...
                       <p> ${{$tournament->fees * $marketprice}} | {{$tournament->fees}} BTP <i class="fa fa-angle-right"></i> </p>
                    </div>
                </div>
                <div>
                  <div class="code-container" >
                    ...
                    <label class="switch">
                      ...
                      <div id="invitation_code_div" style="display:none">
                          <input type="text" class="editbox1" onblur="addInvitationCode()" id="invitation_code" name="invitation_code" value="" >
                        </div>
                  </div>
                </div>
...
$(function(){
  ...
  $( "a" ).each(function() {
     if ($(this).attr("href") !== "#") {
	   ...
	   console.log('target_URL',target_URL.split('#')[1]);
       if (target_URL == current_page_URL) {
          $('nav').parents('li, ul').removeClass('active');
		  $('.nav a').filter(function(){return this.href==location.href}).parent().addClass('active').siblings().removeClass('active')
		  $('.tab-content div').filter(function(){return this.href==location.href}).parent().addClass('active').siblings().removeClass('active')
		  if (document.querySelector('.tab-content div.active') !== null) {
			document.querySelector('.tab-content div.active').classList.remove('active');
		  }
		$("#"+target_URL.split('#')[1]).addClass('active in');  
