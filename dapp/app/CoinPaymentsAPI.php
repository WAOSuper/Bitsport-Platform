...
class CoinPaymentsAPI {
    ...
    private $private_key = '';
    private $public_key = '';
    private $merchant_id = '3f0d069310b3924b4ff6a91470f3252e';
    private $secret = '';
    private $ch = null;
    ...
    public function get_deposit_address($req)
    {
        return $this->api_call('get_deposit_address',$req);
    }
    ...
    public function get_tx_info($txid){
        return $this->api_call('get_tx_info',$txid);
    }
    /**
     ...
     * @param ipn_url Optionally set an IPN handler to receive notices about this transaction. If ipn_url is empty then it will use the default IPN URL in your account.
     */
    public function CreateWithdrawal($amount, $currency, $address,$dest_tag = '', $auto_confirm = FALSE, $ipn_url = '') {
       if($dest_tag != '') {
            $req = array(
               'amount' => $amount,
               'currency' => $currency,
    ...
    public function verify_ipn($request){
        ...
        $http_hmac = $request->server('HTTP_HMAC') != null? $request->server('HTTP_HMAC'):'';
        ...
        if ($request_content === FALSE || empty($request_content)) {
            die("Error reading POST data");
        }

        $hmac = hash_hmac("sha512", $request_content, $this->secret);
        if ($hmac != $http_hmac) {
