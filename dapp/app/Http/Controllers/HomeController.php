...
class HomeController extends Controller
{
    ...
    { 
        ...
        //   'X-CMC_PRO_API_KEY: 181f16bd-46df-4f01-8ffd-cd8636227872'
        // ];
        ...
        if($tournaments)
        {
          ...
          {
            $d = $tournament; 
            if ($tournament->status == 0 || $tournament->is_deleted)
              continue;
    	...
    	return view('frontend.home.home', compact('balance', 'bets','banner', 'setting', 'btc_price', 'eth_price', 'ltc_price', 'dash_price', 'video', 'oddMaster','team_data', 'tournaments', 'user', 'matches'));
    }
    public function events($category)
    {
       ...
       // $eventmaster = EventMaster::with('events.cat','events.subcat','events.subsubcat','events.team1','events.team2','events.oddcount')->where('id', $event[0]['event_master_id'])->get();  
        ...
        if($category == "challenges") {
          $status = array_keys(\config('challenge.status'));
    ...
    public function getEvent($category=null,$day=null)
    {
        ...
        if($category != null){
          $eventmaster = EventMaster::with('events.cat','events.subcat','events.subsubcat','events.team1','events.team2','events.oddcount')
                        ...
                        // ->whereIn('id', function ($q) use ($cat) {
                        //     $q->from('events');
                        //     $q->whereIn('cat_id', $cat->id)->whereIn("event_master_id", "events.id")->where('live','!=', 2);
                        // });
   ...
   public function getLastNDays($days, $format = 'Y-m-d'){
    ...
    public function event_odds($id)
    {
      ...
      // {
        ...
        if(isset($user->id) && !empty($team_data))
        {
          $follow = Follow::where('follower_id',$user->id)->where('following_id',$team_data->user->id)->first();
          if($team_data->match_id){
            $match = Match::find($team_data->match_id);
            $eventmaster = EventMaster::with('livep2pevents.cat','livep2pevents.subcat','livep2pevents.subsubcat','livep2pevents.P2PTeam1','livep2pevents.P2PTeam2','livep2pevents.oddcount.oddMasterName')->where('id', 97)->first();
          }
        }
        elseif(!empty($team_data)) {
            $follow = array();
            $match = Match::find($team_data->match_id);
            $eventmaster = EventMaster::with('livep2pevents.cat','livep2pevents.subcat','livep2pevents.subsubcat','livep2pevents.P2PTeam1','livep2pevents.P2PTeam2','livep2pevents.oddcount.oddMasterName')->where('id', 97)->first();
        ...
        // $odds = Odd::with('event.Team1')->where('event_id',$id)->where('team_id', $odd[0]['event']->team1->id)->get();
            ...
            $balance = (Sentinel::check()) ? User::where('id',Sentinel::getUser()->id)->pluck('mbtc')->first() : 0;
            $bets = (Sentinel::check()) ? Bet::where('user_id',Sentinel::check()->id)->where('status',0)->get() : 0;
            return view('frontend.home.event_odd',compact('team_data','balance','bets','video', 'oddMaster', 'odds', 'role', 'followLabel', 'id', 'user', 'event', 'match', 'eventmaster'));
        // }
        // else
        // {
    ...
    public function getPtoPLiveEvent()
    {

      return $eventmaster = EventMaster::with('livep2pevents.cat','livep2pevents.subcat','livep2pevents.subsubcat','livep2pevents.P2PTeam1','livep2pevents.P2PTeam2','livep2pevents.oddcount.oddMasterName')->where('id', 97)->get();

      return Event::with('cat','subcat','subsubcat','team1','team2','eventMaster','oddcount.oddMasterName')->where('live',1)->where('winloss', 0)->where('featured',0)->where('is_deleted', 0)->orderBy('created_at','desc')->get();
    }
